    ...
    <div class="huzhuan">
        ...
        <div class="pays">
            <div class="num flex flex_between ali_center">
                <div class="title">转让竞拍订单ID</div>
                <input v-model="id" type="text" placeholder="输入可转让竞拍订单ID" disabled />
                <!-- <div class="tishi">可用 {{usdt}} USDT 可提现 {{usdt_with}} USDT</div> -->

            </div>
             <div class="num flex flex_between ali_center">
                <div class="title">转让账号</div>
                <input v-model="roomid" type="text" placeholder="输入转让账号" />
            </div>
        ...
        <div class="submit" :class="{on: id && roomid}" @click="submit">确认</div>
        <van-action-sheet
            v-model="show"
            :actions="actions"
            @select="onSelect"
...
export default {
    ...
    methods: {
        ...
        async submit() {
            this.showPwd = true;
            if(!this.id) return Toast("输入竞拍订单ID!")
            if(!this.roomid) return Toast("输入账号!")
            
            let res = await $ajax('auctionauction1zhuan_auction', {
                // getType: 2,
                id: this.id,
